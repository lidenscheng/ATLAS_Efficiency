...
#ifdef EffStudy_cxx
EffStudy::EffStudy(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("run00302393_DESDM_MCP_stableBeam_merged_extValidNtuple.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("run00302393_DESDM_MCP_stableBeam_merged_extValidNtuple.root");
      }
...
Int_t EffStudy::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t EffStudy::LoadTree(Long64_t entry)
{
...
void EffStudy::Show(Long64_t entry)
{
...
Int_t EffStudy::Cut(Long64_t entry)
{
